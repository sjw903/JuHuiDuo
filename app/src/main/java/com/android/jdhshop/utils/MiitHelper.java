...
public class MiitHelper implements IIdentifierListener {
    ...
    public static final int HELPER_VERSION_CODE = 20211018; // DemoHelper版本号
    private final AppIdsUpdater appIdsUpdater;
    private boolean isCertInit = false;

    public boolean isSDKLogOn = true;          // TODO （1）设置 是否开启sdk日志
    public static final String ASSET_FILE_NAME_CERT = "com.android.jdhshop.cert.pem";  // TODO （2）设置 asset证书文件名
    ...
    public void onSupport(IdSupplier supplier) {
        ...
        String idsText= "CaiNiaoApplicationOnIdsValid support: " + (isSupported ? "true" : "false") +
        ...
        if (!isSupported){
            ...
            appIdsUpdater.supportOaID(oaid);
        }
    }
    ...
    public void getIMEI(){
        // 获取手机设备的唯一码(IMIE )
        final TelephonyManager TelephonyMgr = (TelephonyManager) (mContext).getSystemService(Context.TELEPHONY_SERVICE);
//        动态获取权限

        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP_MR1) {
            imei = TelephonyMgr.getDeviceId();
            appIdsUpdater.onIdsValid(imei);
            appIdsUpdater.supportOaID(imei);
        } else {
            Acp.getInstance(mContext).request(new AcpOptions.Builder().setPermissions(android.Manifest.permission.READ_PHONE_STATE).build(), new AcpListener() {
                @Override
