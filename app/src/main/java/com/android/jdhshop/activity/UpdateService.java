...
import me.shenfan.updateapp.UpdateProgressListener;
...
public class UpdateService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (!startDownload && intent != null) {
            ...
            if (DEBUG) {
                ...
                LogUtils.d(TAG, "icoResId: " + icoResId);
                LogUtils.d(TAG, "icoSmallResId: " + icoSmallResId);
                LogUtils.d(TAG, "storeDir: " + storeDir);
                LogUtils.d(TAG, "updateProgress: " + updateProgress);
                LogUtils.d(TAG, "downloadNotificationFlag: " + downloadNotificationFlag);
                LogUtils.d(TAG, "downloadErrorNotificationFlag: " + downloadErrorNotificationFlag);
                LogUtils.d(TAG, "downloadSuccessNotificationFlag: " + downloadSuccessNotificationFlag);
                LogUtils.d(TAG, "isSendBroadcast: " + isSendBroadcast);
            }
    ...
    private static class DownloadApk extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            if (DEBUG) {
                LogUtils.d(TAG, "download url is " + downloadUrl);
                LogUtils.d(TAG, "download apk cache at " + file.getAbsolutePath());
            }
            ...
            try {
                ...
                if (DEBUG) {
                    LogUtils.d(TAG, "download status code: " + httpConnection.getResponseCode());
                }
        ...
        protected void onProgressUpdate(Integer... values) {
            ...
            if (DEBUG) {
                LogUtils.d(TAG, "current progress is " + values[0]);
            }
            UpdateService service = updateServiceWeakReference.get();
            if (service != null) {
                service.update(values[0]);
                Builder.updateProgress=values[0];
            }
        }
    ...
    public static class Builder {
        ...
        public static int updateProgress = UPDATE_NUMBER_SIZE;   //update notification progress when it add number
        private String storeDir;          //default sdcard/Android/package/update
        private int downloadNotificationFlag;
        private int downloadSuccessNotificationFlag;
        private int downloadErrorNotificationFlag;
        ...
        protected Builder(String downloadUrl) {
            ...
            LogUtils.d("huanghgg",downloadUrl);
        }
