...
//        默认空值，获取全部
        ...
        HttpUtils.post(Constants.GET_ORDER_LIST_NEW,InvalidOrderFragment.this, params, new onOKJsonHttpResponseHandler<OrderGuestNewBean>( new TypeToken<Response<OrderGuestNewBean>>() {
        }) {
            @Override
    ...
    private void getPdd(){
        ...
        HttpUtils.post(Constants.GET_PDD_ORDER,InvalidOrderFragment.this, params, new onOKJsonHttpResponseHandler<PddOrderBean>( new TypeToken<Response<PddOrderBean>>() {
        }) {
            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                showToast(responseString);
    ...
    private void getJd(){
        ...
        params.put("type",MyOrderActivity.CURRENT_ORDER_TYPE);
        params.put("order_sn",MyOrderActivity.order_sn);
        HttpUtils.post(Constants.GETJD_ORDERLIST, InvalidOrderFragment.this,params, new onOKJsonHttpResponseHandler<JdOrderBean>(new TypeToken<Response<JdOrderBean>>() {
        }) {
            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                showToast(responseString);
            ...
            public void onSuccess(int statusCode, Response<JdOrderBean> datas) {
                if (datas.isSuccess()) {
                    ...
                    for(int i=0;i<list.size();i++){
                        if(Integer.valueOf(list.get(i).getOrder_status())<=14){
                            listjd.add(list.get(i));
                        }
                    }
                    if(list.size()==0){
                        hasJdMore=false;
    ...
    private void getWph(){
        ...
        HttpUtils.post(Constants.APP_IP+"/api/WphOrder/getList", InvalidOrderFragment.this,params, new onOKJsonHttpResponseHandler<WphOrderBean>(new TypeToken<Response<WphOrderBean>>() {
        }) {
            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                showToast(responseString);
