...
import com.android.jdhshop.activity.JHSActivity;
import com.android.jdhshop.bean.JdGoodsBean;
...
public class JDAdapterList extends CommonAdapter<MyGoodsResp> {
    ...
    public JDAdapterList(Context context, int layoutId, List<MyGoodsResp> datas) {
        super(context, layoutId, datas);
    }

    @Override
    protected void convert(ViewHolder holder, final MyGoodsResp item, int position) {
        ...
        tx2_2.setText("原价:¥" + (item.pingGouInfo != null ? item.priceInfo.getPrice() : 0));
        //内部
        TextView tx3 = holder.getView(R.id.tx3);
        if (item.pingGouInfo == null || item.couponInfo.getCouponList().length == 0) {
            tx3.setText(0 + "");
        } else {
        ...
        if (item.commissionInfo == null) {
            ...
            comm = df.format(Double.valueOf(tx2.getText().toString().replace("¥", "")) * Double.valueOf(df.format(item.commissionInfo.getCommissionShare() / 100)) * Double.parseDouble(df.format((float) SPUtils.getIntData(mContext, "rate", 0) / 100)));
        }
