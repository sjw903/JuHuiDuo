...
public class OrderallFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
       ...
       mview = inflater.inflate( R.layout.fragment_orderall, container, false );
        init();
        return mview;
    }
    public void init(){
        listView = mview.findViewById( R.id.orderall_listview );
    ...
    private void postgetaddresslist(String search) {
        ...
        HttpUtils.post1( Constants.getOrderList, OrderallFragment.this,requestParams, new TextHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
    ...
    private void postqxorder() {
        ...
        RequestParams requestParams = new RequestParams();
        requestParams.put( "token",token );
        requestParams.put( "order_detail_id",orderid);
        HttpUtils.post1( Constants.cancel,OrderallFragment.this, requestParams, new TextHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
                    ...
                    orderlistAdapter.setsubClickListener( new OrderlistAdapter.SubClickListener() {
                        ...
                        public void OntopicClickListener(View v, String detail, int posit) {
                            if (detail.equals( "qxdd" )){//取消订单
                                orderid = orderlist.get( posit ).order_detail_id;
                                dialog();
                            }else if (detail.equals( "fk" )){//付款

                                Intent payment = new Intent( getActivity(), MerRechargeActivity.class );
                                payment.putExtra( "orderid",orderlist.get( posit ).order_id );
                                payment.putExtra( "allprice",orderlist.get( posit ).allprice );
                                payment.putExtra( "title",orderlist.get( posit ).goods_name );
                                payment.putExtra( "pid","" );
...
//                                startActivity( payment );
                                ...
                                orderid = orderlist.get( posit ).order_id;
                                postqxorder();
